...
if __name__ == "__main__":
    ...
    print("Total frames number: ", frame_numbers)
    ...
    mean_fps_time = round(np.sum(eps_time)/int(frame_numbers), 2)
    print(mean_fps_time)
    plt.figure(figsize=(10,10))
    plt.title('FPS Plot, mean: {}'.format(mean_fps_time))
    plt.plot(list(range(1, int(len(eps_time)))), eps_time, '-')
    plt.xlabel('Frame')
    plt.ylabel('Real-time FPS')
    plt.grid()
